...
-- ============================================================
-- RPC: debitar_maxbank_salario
-- ============================================================
-- Chamada pelo MaxBank quando o colaborador lê um QR PIX gerado
-- pelo PDV e opta por pagar com o saldo_salario da carteira dele
-- em maxbank_contas.
--
-- Argumentos batem 1:1 com o cliente (src/lib/pagamento.ts do MaxBank):
--   p_valor, p_descricao, p_pix_pendente_id.
--
-- Idempotência: UNIQUE parcial em maxbank_transacoes garante que a
-- mesma pix_pendente_id não seja debitada duas vezes na carteira
-- 'salario' (origem = 'pix_pendente'). Retry retorna status
-- 'ja_debitado' + saldo atual.
-- ============================================================
